    ...
    static class Program
    {
        ...
        static void Main()
        {
            ...
            if (File.Exists(@"Brave Dev x86\Brave.exe"))
            {
                ...
                if (!File.Exists(applicationPath + "\\Brave Dev x86\\Profile.txt"))
                {
                    Application.EnableVisualStyles();
                    Application.SetCompatibleTextRenderingDefault(false);
                    Application.Run(new Form1());
                    String Arguments = File.ReadAllText(applicationPath + "\\Brave Dev x86\\Profile.txt") + sb.ToString();
                    if (Arguments.Contains("--user-data-dir="))
                    {
                        ...
                        Process.Start(applicationPath + "\\Brave Dev x86\\Brave.exe", Arguments3.Replace("'", ""));
                    }
                    else
                    {
                        Process.Start(applicationPath + "\\Brave Dev x86\\Brave.exe", Arguments);
                    }
                }
                else
                {

...
namespace Brave_Launcher
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            if (File.Exists(applicationPath + "\\Brave\\Brave.exe"))
            {
                ...
                if (!File.Exists(applicationPath + "\\Brave\\Profile.txt"))
                {
                    Application.EnableVisualStyles();
                    Application.SetCompatibleTextRenderingDefault(false);
                    Application.Run(new Form1());
                    string Arguments = File.ReadAllText(applicationPath + "\\Brave\\Profile.txt") + sb.ToString();
                    if (Arguments.Contains("--user-data-dir="))
                    {
                        ...
                        Process.Start(applicationPath + "\\Brave\\Brave.exe", Arguments3.Replace("'", ""));
                    }
                    else
                    {
                        Process.Start(applicationPath + "\\Brave\\Brave.exe", Arguments);
                    }
                }
                else
                {
